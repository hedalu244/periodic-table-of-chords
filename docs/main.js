(() => {
  ...
  function isSameChord(chordId1, chordId2) {
    ...
    if (T_group.includes(chordId1) && T_group.includes(chordId2) || D_group.includes(chordId1) && D_group.includes(chordId2) || S_group.includes(chordId1) && S_group.includes(chordId2)) {
      return true;
    }
  ...
  function generateBasicChord(name) {
    ...
    switch (quality) {
      ...
      // Minor triad
      case "dim":
        return transposeChord([0, 3, 6], rootPitchClass);
      // Diminished triad
      case "7":
  ...
  // src/chord-table.ts
  var DEFAULT_CHORD_TABLE_VISIBILITY = {
  ...
  function generateChordTable(visibility) {
    const chordTable = [];
  ...
  function generateDiminishedChords() {
    const chordTable = [];
    for (let i = 0; i < 12; i += 1) {
  ...
  var TorusRenderer = class {
    constructor(params) {
      this.chordTable = [];
      this.chordById = /* @__PURE__ */ new Map();
  ...
  // src/visibility-controls.ts
  function readVisibilityState(diminishedSeventhInput, diminishedInput, halfdiminishedInput, minorTriadInput, minorSeventhInput, majorTriadInput, dominantSeventhInput) {
    return {
  ...
  function setupVisibilityControls(onChanged) {
